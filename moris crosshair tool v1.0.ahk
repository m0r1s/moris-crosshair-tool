...
global settingsGui := Gui("-Resize", "moris crosshair tool v1.0")
...
outlineGroup := settingsGui.Add("GroupBox", "x164 y40 w136 h160", "Outline Settings")

outlineEnabledCB := settingsGui.Add("CheckBox", "x172 y60 w120 h20 vOutlineEnabledCB Checked" (settings["outlineEnabled"] ? 1 : 0), "Enable Outline")
outlineEnabledCB.OnEvent("Click", UpdateOutlineSettings)

settingsGui.Add("Text", "x172 y90 w60 h20", "Color:")
outlineColorDropdown := settingsGui.Add("DropDownList", "x172 y110 w120 h200 vOutlineColor Choose1", [
...
settingsGui.Add("Text", "x172 y140 w80 h20", "Thickness:")
outlineThicknessEdit := settingsGui.Add("Edit", "x172 y160 w50 h22 vOutlineThicknessEdit Number", settings["outlineThickness"])
...
otherGroup := settingsGui.Add("GroupBox", "x164 y205 w136 h84", "Other")

rightClickToggleCB := settingsGui.Add("CheckBox", "x172 y225 w120 h20 vRightClickToggleCB Checked" (settings["rightClickHide"] ? 1 : 0), "Right Click Toggle")
...
UpdateSettings(*) {
    ...
    if RegExMatch(colorDropdown.Text, "\(([0-9A-F]{6})\)", &match) {
        settings["color"] := match[1]
    }
...
UpdateOutlineSettings(*) {
    ...
    if RegExMatch(outlineColorDropdown.Text, "\(([0-9A-F]{6})\)", &match) {
        settings["outlineColor"] := match[1]
    }
...
LoadProfile(*) {
    ...
    try {
        ...
        items := ["Green (00FF00)", "Red (FF0000)", "Blue (0000FF)", 
                 "White (FFFFFF)", "Black (000000)", "Yellow (FFFF00)",
                 "Cyan (00FFFF)", "Magenta (FF00FF)", "Orange (FFA500)"]
        
        loop items.Length {
            if InStr(items[A_Index], selectedColor) {
                colorDropdown.Value := A_Index
                break
            }
        }

        selectedOutlineColor := "(" settings["outlineColor"] ")"
        outlineItems := ["Black (000000)", "White (FFFFFF)", "Red (FF0000)", 
                        "Blue (0000FF)", "Green (00FF00)", "Yellow (FFFF00)",
                        "Cyan (00FFFF)", "Magenta (FF00FF)", "Orange (FFA500)"]
        
        loop outlineItems.Length {
            if InStr(outlineItems[A_Index], selectedOutlineColor) {
...
SaveProfileList() {
    profileNames := ""
    for profileName, _ in settings["profiles"].OwnProps() {
